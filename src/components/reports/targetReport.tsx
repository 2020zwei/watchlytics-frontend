...
const TargetReport = () => {
  ...
  const { data, isLoading, isError } = usePieReports({ segment: "remaining" });

  if (isLoading) return <LoaderWidget />
  if (isError) return <Notfound label='Failed to fetch target report' />
