...
import { useCreateTransaction, useTransactionById, useUpdateTransaction } from '@/hooks/useTransaction';
import { useCategores } from '@/hooks/useCategory';
import { useCustomerList } from '@/hooks/useCustomerHooks';
...
const AddTransaction = () => {
    ...
    const { data: categoryData, isLoading: isCategoryLoading } = useCategores(id as string);
    const { data: customerData, isLoading: isCustomerLoading } = useCustomerList();
    const { data: tradeData, isLoading: isTradeLoading } = useTransactionById(id as string);
    ...
    const onSubmit = async (data: TradeFormValues) => {
        const transaction_items = product?.map((item) => ({
            product: item.id,
        ...
        const mutation = id
            ? updateTransaction({ id, formData: payload })
        ...
        try {
            ...
            const errors = err?.response?.data?.details?.non_field_errors || ["Something went wrong"];
    ...
    useEffect(() => {
        if (categoryData?.status === 200) {
            ...
            const filtered = data?.results.filter((el: any) => el?.quantity > 0)
            const options = (id ? data?.results : filtered)?.map((item: any) => ({
                value: item.id,
    ...
    useEffect(() => {
        if (customerData?.status === 200) {
            const options = customerData?.data?.results?.map((item: any) => ({ value: item?.id, label: item?.name }))
    ...
    }, [tradeData, categoryData]);

    if (isTradeLoading || isCategoryLoading || isCustomerLoading) {
